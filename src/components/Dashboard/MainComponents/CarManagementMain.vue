        ...
        <div class="table-container">
            <v-table class="mt-8">
                ...
                >
                    ...
                    <td>{{ car.isInBussiness ? "Đang kinh doanh" : "Dừng kinh doanh"}}</td>
                    <td>
                        <div class="d-flex function-car-container">
                            <span class="mdi mdi-eye detailCar" title="Xem chi tiết thông tin xe" v-on:click="handleShowDetail(car._id)"></span>
                            <span class="mdi mdi-delete-forever deleteCar" title="Xóa xe" v-on:click="handleDeleteCar(car._id)"></span>
                        </div>
                    </td>
                </tr>
                </tbody>
            </v-table>
            <PaginationPage class="pagination" :length="length" v-model:page="currentPage" @update:page="handlePageUpdate"></PaginationPage>
        </div>
    </v-container>
</template>
...
export default {
    ...
    setup(){
        ...
        const handleShowDetail = (carId) =>{
            store.commit('dashBoard/setCurrentComponentMain', {component:"UpdateCarMain", carId});
        }
        return{cars, filter, formatter, handlePageUpdate, currentPage, length, handleDeleteCar,
            handleShowDetail
